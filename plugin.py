...
<plugin key="FliprLite" name="ZZ - Flipr Basic Analyzer" author="Erwanweb" version="1.0" externallink="https://my.goflipr.com">
    ...
    <params>
        ...
        <param field="Mode5" label="data verification period" width="400px">
            <options>
                <option label="Quick - Every min" value="1"/>
                <option label="Medium - 15 mins" value="15"/>
                <option label="Normal - 30 mins" value="30"  default="true"/>
                <option label="Quiet - 1 hour" value="60"/>
                <option label="slow - 3 hour" value="180"/>
                <option label="2x a day" value="720"/>
            </options>
        </param>
...
class BasePlugin:
    ...
    def onHeartbeat(self):
        now = datetime.now()
        Domoticz.Debug(f"Datas period set at {self.DataPeriod}, Timed Out Period set at {self.DataTimedOutPeriod} hours")
        ...
        try:
            ...
            if not DataTimedOut :
                ...
                if not self.MesureId == self.PreviousMesureId :
                    ...
                    Domoticz.Debug(f"Flipr Calibration -- Ph7 is : {self.PhCalib} -- Redox offset is {self.RedoxOffset}")
                    ph = ph - self.PhCalib
