...
class BasePlugin:
    ...
    def scrape_data(self):
        try:
            url = "https://my.goflipr.com/fleet.php"  # ✅ Fleet View page = tableau HTML complet
            headers = {
                "User-Agent": "Mozilla/5.0"
            }
            response = self.session.get(url, headers=headers, timeout=30)
            if response.status_code != 200:
                Domoticz.Error(f"Failed to load Flipr fleet view: HTTP {response.status_code}")
                return
            ...
            if pool:
                ...
                Domoticz.Error("Could not find pool data in Fleet page — session might have expired.")
