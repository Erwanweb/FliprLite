...
class BasePlugin:
    ...
    def login(self):
        ...
        payload = {
            ...
            "accept": "on"  # ✅ coche « J'accepte » virtuellement
        }
        headers = {
            "User-Agent": "Mozilla/5.0"
        }
        try:
            # ✅ Force cookie consent explicitement
    ...
    def scrape_data(self):
        try:
            url = "https://my.goflipr.com/welcome.php"  # ✅ c'est bien welcome.php pour toi
            headers = {
                "User-Agent": "Mozilla/5.0"
            }
            response = self.session.get(url, headers=headers, timeout=30)
            if response.status_code != 200:
                Domoticz.Error(f"Failed to load Flipr welcome page: HTTP {response.status_code}")
                return
            ...
            if pool:
                ...
                Domoticz.Error("Could not find pool data in welcome page — session might have expired.")
