    ...
    def scrape_data(self):
        try:
            ...
            if "DOCTYPE" in response.text:
                Domoticz.Error("Flipr returned HTML instead of JSON — session expired.")
                return
            ...
            Domoticz.Log(f"Flipr JSON raw: {data_list}")
            ...
            current = first_entry.get("Current")
            if not current:
                Domoticz.Error("No 'Current' data available in JSON entry[0].")
                return

            temp_raw = current.get("Temperature", 0)
            ph_raw = current.get("PH", {}).get("Value", 0)
            redox_raw = current.get("OxydoReductionPotentiel", {}).get("Value", 0)

            temp = round(float(temp_raw), 1)
            ph = round(float(ph_raw), 2)
            redox = int(round(float(redox_raw), 0))

            Domoticz.Log(f"Scraped JSON (Current): Temp={temp}°C, pH={ph}, Redox={redox} mV")
