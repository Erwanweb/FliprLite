...
<plugin key="FliprLite" name="ZZ - Flipr Basic Analyzer" author="Erwanweb" version="1.0" externallink="https://my.goflipr.com">
    ...
    <params>
        ...
        <param field="Mode2" label="PH 7 calibration (ex:7.34)" width="50px" required="true" default=""/>
        <param field="Mode3" label="Redox offset (ex -50)" width="50px" required="true" default=""/>
...
class BasePlugin:
    ...
    def scrape_data_raw(self):
        try:
            ...
            Domoticz.Log(f"Scraped RAW JSON: Temp={temp}°C, pH={ph}, Redox={redox} mV")

            # ✅ calibration correct :
            self.PhCalib = 7 - float(self.PhLastCalib)
            ph = ph - self.PhCalib
            redox = int(round(float(redox_raw), 0))  # Sans virgule
            Domoticz.Log(f"Flipr Calibrated values: Temp={temp}°C, pH={ph}, Redox={redox} mV")
