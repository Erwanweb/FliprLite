...
class BasePlugin:
    ...
    def scrape_data(self):
        try:
            url = f"https://my.goflipr.com/dash-my-flipr.php?serial={self.serial}&placeid={self.placeid}"
            headers = {
                "User-Agent": "Mozilla/5.0"
            }
            ...
            if "DOCTYPE" not in response.text:
                Domoticz.Error("Flipr returned non-HTML content, session expired?")
                return

            from bs4 import BeautifulSoup
            soup = BeautifulSoup(response.text, "html.parser")

            # ✅ Exemple : cherche la première balise contenant '°C' pour la température
            temp_element = soup.find(string=lambda t: "°C" in t)
            temp = float(temp_element.strip().replace("°C", "").replace(",", "."))
            temp = round(temp, 1)

            # ✅ Exemple : cherche le premier élément qui ressemble à un pH (approx)
            ph_element = soup.find(string=lambda t: "." in t and len(t.strip()) <= 5)
            ph = round(float(ph_element.strip().replace(",", ".")), 2)

            # ✅ Exemple : cherche la balise contenant 'mv' pour le redox
            orp_element = soup.find(string=lambda t: "mv" in t.lower())
            redox = int(orp_element.strip().lower().replace("mv", "").replace(" ", ""))

            Domoticz.Log(f"Scraped HTML: Temp={temp}°C, pH={ph}, Redox={redox} mV")
